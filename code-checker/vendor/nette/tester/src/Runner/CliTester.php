...
Options:
    ...
    --tmp <path>                Path to temporary directory. Default by sys_get_temp_dir().
...
XX
		...
		if ($this->options['--tmp'] === null) {
			if (($temp = sys_get_temp_dir()) === '') {
				...
				$this->options['--tmp'] = rtrim($real, DIRECTORY_SEPARATOR);
			}
		}
	...
	private function createRunner(): Runner
	{
		...
		if ($this->options['--tmp'] !== null) {
			$runner->setTempDirectory($this->options['--tmp']);
		}
